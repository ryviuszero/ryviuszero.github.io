---
layout: post
title: 用 Codex 做复杂项目开发：我踩过的坑和真正有效的方法
...
现在自己做完一遍，大概也知道为什么了。因为这种体量的经验，很难被压缩成一个几千字的“教程”。很多文章最后都会变成提纯过的方法论，看起来很正确，但你真的开项目时，还是不知道第一步该怎么做、什么时候该停下来、哪里必须自己判断。

所以这篇文章更像是给一个月前的我写的备忘录。它不是要证明 Codex 多强，也不是要说 AI 可以替你完成所有开发。相反，我这次最大的感受是：AI 编程工具越强，越需要你有自己的流程。不然它确实能写得很快，但也能很快把项目带偏。

我自己也还在摸索，下面的内容肯定不是标准答案。更准确地说，它只是我用 Fast Sub 这个项目踩出来的一套阶段性经验。

## 1. 我最大的变化：不再把 AI 编程当自动驾驶

刚开始用 AI 写代码的时候，很容易有一种错觉：只要把需求说清楚，它就能一路往前开。

写临时脚本时，这个感觉确实成立。你说我要处理一个文件、调一个 API、生成一段测试数据，它写出来，你跑一下，修几个小问题，事情就结束了。

但 Fast Sub 不是这种项目。它从 Python CLI 开始，后来加了 Go product core、daemon/job API，再到 Electron 桌面端、打包、模型下载、真实 provider、QA smoke 和开源文档。项目一旦到这个规模，我越来越明显地感觉到，AI 编程不是自动驾驶。

它更像一个执行力很强、知识面很广、但非常需要上下文和边界的协作者。你告诉它“现在只做 mock，不要接真实 daemon”，它可以很好地执行。你告诉它“这个 JSON schema 和退出码不能破坏”，它也能尽量遵守。但如果这些约束只存在你脑子里，或者散落在前几天的聊天记录里，那它迟早会忘。

我后来反复想到一句话：

> 复杂度不会消失，只会转移。

你不在 MVP 阶段控制范围，复杂度就会转移到后面的返工里。你不在文档里固定上下文，复杂度就会转移到每个新对话的解释成本里。你不在测试和 QA 阶段暴露问题，复杂度就会转移到用户真正使用的时候。

这也是我现在看 Codex 的方式：它不是一个替你开车的司机，更像是一个很强的队友。你要给它地图、边界、验收标准，还要知道什么时候该踩刹车。

## 2. MVP：先跑通最小闭环，不要一上来造完整系统

我这里说的 MVP，不是一个很正式的产品概念，只是第一个能跑通核心流程的最小版本。

从零做项目时，我一开始也会有冲动：既然 AI 写代码这么快，那是不是可以一上来就把完整系统规划好？功能、架构、文档、测试、UI，全都一次性做出来。

后来发现，这个想法对个人项目不太现实。成熟团队可以在前期做大量设计，因为他们有人力、有评审、有经验，也有比较稳定的需求输入。但个人项目很多时候是在做的过程中才逐渐想清楚。AI 可以帮你讨论方案，但不能替你做所有产品判断。
...
不过 MVP 不等于让 AI 直接开写。这个坑我觉得新手特别容易踩。Claude Code、Codex 都很容易在你提完需求后立刻开始写代码，它们太积极了。如果只是写一个临时脚本，这没问题。但如果是一个稍微严肃一点的项目，从这里开始就可能失控。
...
这里 prompt 没有多神秘。至少到 2026-05-22 这个阶段，我的感受是，与其迷信某句咒语，不如把需求、约束、参考项目和你自己的疑问讲清楚。必要时一定让 AI 联网查一下。Codex 有时会给出不是最新的 Next.js、GitHub Actions、Node 版本配置，这类问题如果不查，后面会变成莫名其妙的 warning 或构建失败。

MVP 文档出来后，我会继续让 AI 扮演 project manager，把项目拆成几轮 round。每轮写清楚目标、范围和验收方式。后面执行时就简单很多：先完善 round 文档，再切分支实现，最后按文档验收。

这一步看起来慢，但它是在帮后面省时间。

## 3. SPEC 和文档：别让 AI 在模糊需求里自由发挥
...
这对 AI 特别重要。因为 AI 很擅长“顺便”。你让它修一个 UI 问题，它可能顺便改了状态管理。你让它补一个 daemon API，它可能顺便调整了 renderer contract。很多时候它不是故意乱来，只是它会根据局部上下文判断“这样更合理”。但项目里有些东西不是局部合理就能改的，比如 CLI 参数、JSON schema、退出码、provider contract、远程上传确认逻辑。
...
所以 SPEC 里我最看重的其实不是“要做什么”，而是“不能做什么”。
...
有了这些边界，AI 的发挥空间反而更稳定。它知道这一轮只该解决这一轮的问题，遇到超出范围的东西就记录 follow-up，而不是直接动手。

我现在的习惯是：大的实现前先写 SPEC，自己审一遍，再让另一个对话或模型审一遍。不是为了写得多漂亮，而是为了在动代码前先把模糊的地方暴露出来。

## 4. 上下文管理：把项目记忆从聊天窗口搬出来

Fast Sub 前几轮基本是靠对话推进的。一开始没什么问题，项目还小，AI 也能跟得上。但后面继续做时，问题开始变明显。

最典型的情况是 project manager 对话忘记了自己的角色，开始改代码。还有些对话会忘记之前完成了什么，或者不知道哪些 contract 是不能动的。你提醒它一次，它会恢复正常；过一段时间，又可能偏掉。

这不是某个模型特别差，而是 LLM 对话本来就不是稳定记忆。Claude Code、Codex 这些工具会帮你维护上下文，但上下文终究有限。对话长了会压缩，会丢细节，也会混入很多已经不重要的信息。
...
这些文件不是为了让仓库看起来更“工程化”，而是为了让新对话能接得上。实现对话开始前，先读这些上下文。改完代码后，再更新 tracker。这样项目记忆就不再完全依赖某一个聊天窗口。
...
我后来也意识到，上下文管理不是把所有东西都塞给 AI。上下文太少，它会误解；上下文太多，它会变慢，也可能抓不住重点。所以比较好的方式，是把长期稳定的信息写进项目文档，把当前阶段的信息写进 SPEC 和 tracker。

## 5. 多对话协作：把 PM、实现、review、QA 分开

我现在不太会把一个复杂项目塞进一个聊天窗口里。

单个对话越长，越容易变成一锅粥。规划、实现、review、QA、发布文档全混在一起后，AI 很难稳定保持同一个角色。你刚让它做完一轮实现，又让它审自己的代码，再让它规划下一轮，它很容易把上下文搅在一起。
...
这个做法一开始看起来有点麻烦，但后面会舒服很多。PM 对话更像项目状态机，负责把事情拆清楚；实现对话更像临时工，拿到明确任务就做；review 对话则专门挑问题。不同对话之间不靠“你还记得吗”，而靠文档和 tracker 交接。
...
有些 round 还能进一步拆成多个 worktree 并行做。我最多试过 4 个 worktree 同时开发，效率确实很高。但这里有一个前提：任务必须切得足够干净。最后如果几个分支都改同一块状态管理、同一个 contract，合并时就会很痛苦。
...
但自动测试也不是万能的。核心流程还是要自己手动跑一遍。尤其是桌面应用，很多问题只有真实点一遍才知道：按钮是不是能点，弹窗会不会被长文件名撑爆，失败信息用户能不能看懂，任务取消后进程有没有残留。
...
后来我对 AI 重构的判断变得保守了：它很适合拆文件、抽类型、整理模块，但前提是你先定义什么叫“没改坏”。如果没有测试，没有边界，没有“小步改”的节奏，重构很容易变成另一场灾难。

## 7. UI 原型：mock-first 是我踩坑后觉得最值的做法
...
后来我把原型文件下载下来，让 Codex 继续接手修改。这里有一个经验很重要：只给原型代码不够。Codex 只看代码，很难稳定还原视觉效果。最好同时给每个原型页面的截图，让它同时参考结构和最终视觉。

现在回头看，这一步应该已经有更省事的做法了。比如 [Open Design](https://opendesigner.io/)，基本可以理解成 Claude Design 的开源替代方案。它把设计生成流程接到你已有的 coding-agent CLI 上，包括 Codex、Claude Code、Cursor、Gemini、OpenCode 等。这样就不需要像我当时一样，在 Claude Design 和 Codex 之间来回搬原型文件、截图和修改说明。
...
所以现在让我再做类似项目，我还是会先 mock。哪怕后端已经有一部分可用，我也倾向于先用 mock 把用户流程走顺，再逐步替换成真实实现。
...
后来稍微提高效率的办法，是建立 QA 测试表。不要发现一个问题就立刻开一个新对话修一个问题，而是批量记录、批量分类，再让 Codex 按类别处理。这样效率会高很多，也方便回头确认哪些问题已经修、哪些还要复测。
...
这一步很容易被低估。对开发者来说，代码能跑好像就结束了。但对外部用户来说，文档、隐私说明、安装说明和已知限制，都是项目可信度的一部分。AI 很适合帮你生成第一版文档，但文档里的承诺必须自己审。尤其是安装说明、隐私说明、已知限制，不能让 AI 写得太乐观。

Fast Sub 又是本地优先工具，所以隐私边界必须写清楚。远程 provider 不能变成隐式行为，任何上传音频或字幕文本的路径都必须是用户显式选择和确认的。
...
AI 可以帮你更快写代码，但不能让复杂度消失。你不在需求阶段处理，它就在实现阶段出现。你不在实现阶段处理，它就在 QA 阶段出现。你不在 QA 阶段处理，它就在用户那里出现。
...
第二，更早规定目录结构和代码风格。Python 前期文件膨胀后再重构，成本比我预期高。AI 可以帮你重构，但它不擅长替你承担“前期没定边界”的后果。

第三，UI 原型阶段会把截图、状态和文案整理得更细。原型越粗，后面 QA 时补交互细节就越多。尤其是桌面工具，空状态、错误状态、批量任务、取消、失败重试这些页面，最好在 mock 阶段就想清楚。

第四，更早建立 QA 表。不要等到最后一公里问题爆出来，才开始系统记录。QA 表不仅是测试清单，也是和 AI 协作修 bug 的输入格式。
...
这些都不是很炫的技巧，但它们应该能让我少还不少债。

## 11. 总结：少找神奇 prompt，多建立自己的流程

用 Codex 做完这个项目后，我对 AI 编程的看法变了不少。

我不再觉得关键是找到某个万能 prompt。prompt 有用，skills 有用，工具也会越来越强。但复杂项目真正考验的，是你有没有自己的流程。

如果你刚开始用 Codex 或类似工具做项目，我会建议先从这些事开始：

1. 先做 MVP，不要一上来造完整系统。
2. 大任务先写 SPEC，尤其要写清楚不做什么。
3. 把长期上下文放进项目文档，不要只放在聊天记录里。
4. 用多个对话拆分 PM、实现、review、QA。
5. 每轮修改都要有测试和验证，不要只看 AI 说“完成了”。
6. UI 先 mock-first，等流程稳定再接真实后端。
7. QA 问题批量记录、批量修复、批量复测。
8. 开源前单独做文档、隐私、许可证和发布说明收口。

这些听起来都不酷，也没有一句神奇 prompt 来得吸引人。但这是我这次真正觉得有用的东西。

AI 编程不是自动驾驶。它更像是把一个很强的协作者接进你的开发流程。你的流程越清楚，它越能放大你的能力。你的流程越混乱，它也会更快地放大混乱。
